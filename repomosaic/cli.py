...
from repomosaic import __version__
...
from repomosaic.semantic.languages import SUPPORTED_LANGUAGES, normalize_language
...
def _print_language_error(error: ValueError) -> None:
    ...
    message = f"RepoMosaic error: {error}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "commands:\n"
            "  repomosaic init         Configure the current project\n"
            "  repomosaic auth set     Save the user-level API key\n"
            "  repomosaic auth status  Check API-key availability\n"
            "  repomosaic auth clear   Remove the stored API key"
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="repomosaic_output",
        help="Directory for generated files (default: repomosaic_output).",
    )
...
def build_init_parser() -> argparse.ArgumentParser:
    """Create the parser for ``repomosaic init``."""
    parser = argparse.ArgumentParser(
        prog="repomosaic init",
        description="Set up .repomosaic.toml in the current directory.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite an existing .repomosaic.toml file.",
    )
    parser.add_argument(
        "--template",
        action="store_true",
...
def build_auth_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="repomosaic auth",
        description="Manage RepoMosaic's user-level API credential.",
    )
    ...
    commands.add_parser("clear", help="Remove the stored RepoMosaic API key.")
    return parser
    ...
    try:
        if args.auth_command == "set":
            if not _stdin_is_interactive():
                print(
                    "RepoMosaic auth set needs an interactive terminal.",
                    file=sys.stderr,
                )
                return 2
            print("RepoMosaic authentication\n")
            while True:
            ...
            print("\nRepoMosaic API key saved securely.")
            print("\nNext:\n  cd path/to/project\n  repomosaic init\n  repomosaic .")
            return 0
        ...
        if clear_stored_api_key():
            print("RepoMosaic API key removed.")
        else:
            print("No stored RepoMosaic API key found.")
        return 0
    except (CredentialStoreError, ValueError) as error:
        print(f"RepoMosaic error: {error}", file=sys.stderr)
        return 2
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments and arguments[0] == "init":
        ...
        config_path = Path.cwd() / ".repomosaic.toml"
        if config_path.exists() and not init_args.force:
            force_command = (
                "repomosaic init --template --force"
                if init_args.template
                else "repomosaic init --force"
            )
            print(
                "RepoMosaic configuration already exists:\n"
                ".repomosaic.toml\n\n"
                f"Use:\n{force_command}",
                file=sys.stderr,
            )
        ...
        if not init_args.template and not _stdin_is_interactive():
            print(
                "RepoMosaic init needs an interactive terminal.\n"
                "Use 'repomosaic init --template' to create a manual template.",
                file=sys.stderr,
            )
            return 2
    ...
    if llm_requested:
        ...
        if missing:
            print(
                f"RepoMosaic error: missing LLM options: {', '.join(missing)}",
                file=sys.stderr,
            )
            return 2
        ...
        if credential is None:
            print(
                "RepoMosaic error: No API key configured.\n\n"
                "Run:\n  repomosaic auth set\n\n"
