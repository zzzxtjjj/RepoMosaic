"""RepoMosaic Agent API 的本地 stdio MCP 适配器。"""
...
from repomosaic import agent_api
...
def _load_mcp_sdk() -> tuple[type[Any], type[Exception]]:
    """延迟加载可选 MCP SDK，避免影响普通 RepoMosaic 安装。"""
    try:
        from mcp.server import MCPServer
        from mcp.server.mcpserver.exceptions import ToolError
    except ImportError as error:
        raise MCPDependencyError(
            'MCP support is not installed. Install it with: pip install "repomosaic[mcp]"'
        ) from error
...
def create_mcp_server(repository_path: str | Path) -> Any:
    ...
    adapter = RepoMosaicMCPAdapter(repository_path)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="repomosaic-mcp",
        description="Serve RepoMosaic tools for one local repository over stdio.",
    )
    parser.add_argument(
        "repository_path",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"RepoMosaic MCP error: {error}", file=sys.stderr)
        return 2
