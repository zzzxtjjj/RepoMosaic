"""Load and create RepoMosaic's local project configuration."""
...
CONFIG_FILENAME = ".repomosaic.toml"
...
CONFIG_TEMPLATE = f"""# RepoMosaic project configuration
# Use `repomosaic auth set` or REPOMOSAIC_API_KEY; never put API keys in this file.
...
class ProjectConfigError(ValueError):
    """Raised when a local RepoMosaic configuration is invalid."""
...
def load_project_config(directory: Path) -> ProjectConfig:
    ...
    if "api_key" in llm or "api-key" in llm:
        raise ProjectConfigError(
            "API keys are not allowed in .repomosaic.toml; "
            "use `repomosaic auth set` or REPOMOSAIC_API_KEY instead."
        )
